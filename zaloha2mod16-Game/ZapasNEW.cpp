...
void __fastcall TForm1::MA_LButton1Click(TObject *Sender)
{
        if(Match[0]->getCount() != 120){
                ...
                if(AktKolo > (PocetKol - 1)) HO_B_NextWeek->Enabled = false;
...
void __fastcall TForm1::rozpisLigy()
{
        ...
        RozpisLigy[10][1][2] = 5;
        RozpisLigy[10][1][3] = 6;
        RozpisLigy[10][1][4] = 10;
        RozpisLigy[10][1][5] = 11;
}
//---------------------------------------------------------------------------
...
void __fastcall TForm1::updateEnergie()
{
        for(int i = 0; i < 12; i ++) {
                for(int j = 0; j < tymy[i]->getPocetHracu(); j ++) {
                        float plus = tymy[i]->Hraci[j]->getEnergie() / 100;
                        plus = 1 - plus;
                        plus = tymy[i]->Hraci[j]->getEnergie() + 40 * plus;
                        if(plus > 100) plus = 100;
                        tymy[i]->Hraci[j]->setEnergie(plus);
                }
        }
}
//---------------------------------------------------------------------------
